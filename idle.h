...
#define set_idle_delay_freq(freq) ({\
    ...
    IDLE_TIMER_CONTROL_REG = _BV(IDLE_TIMER_WAVEFORM_MODE_BIT_2) | TIMER_16BIT_PRESCALER(freq);\
})

#define set_idle_delay_millis(millis) ({\
    IDLE_OUTPUT_COMPARE_REG = TIMER_16BIT_ALARM_MILLIS(millis);\
    IDLE_TIMER_CONTROL_REG = _BV(IDLE_TIMER_WAVEFORM_MODE_BIT_2) | TIMER_16BIT_PRESCALER_MILLIS(millis);\
})

#define set_idle_delay_micros(micros) ({\
    IDLE_OUTPUT_COMPARE_REG = TIMER_16BIT_ALARM_MICROS(micros);\
    IDLE_TIMER_CONTROL_REG = _BV(IDLE_TIMER_WAVEFORM_MODE_BIT_2) | TIMER_16BIT_PRESCALER_MICROS(micros);\
})
