...
void twi_init(void)
{
  ...
  set_pin_pullup(SDA, true);
  set_pin_pullup(SCL, true);

  // initialize twi prescaler and bit rate
  cbi(TWI_BIT_STATUS_REG, TWI_PRESCALER_BIT_0);
  cbi(TWI_BIT_STATUS_REG, TWI_PRESCALER_BIT_1);
  TWI_BIT_RATE_REG = ((F_CPU / TWI_FREQ) - 16) / 2;
  ...
  TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_ENABLE_ACK_BIT);
}
...
void twi_disable(void)
{
  ...
  set_pin_pullup(SDA, false);
  set_pin_pullup(SCL, false);
}
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  if (true == twi_inRepStart) {
    ...
    twi_inRepStart = false; // Remember, we're dealing with an ASYNC ISR
      ...
      startMicros = micros();
      do {
        TWI_DATA_REG = twi_slarw;
        if((twi_timeout_us > 0ul) && ((micros() - startMicros) > twi_timeout_us)) {
          twi_handleTimeout(twi_do_reset_on_timeout);
          return 0;
        }
      } while(TWI_CONTROL_REG & _BV(TWI_WRITE_COLLISION_FLAG));
    #else
      do {
        TWI_DATA_REG = twi_slarw;
      } while(TWI_CONTROL_REG & _BV(TWI_WRITE_COLLISION_FLAG));
    #endif
    TWI_CONTROL_REG = _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_ENABLE_ACK_BIT) | _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE);  // enable INTs, but not START
  }
  else
    // send start condition
    TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_ENABLE_ACK_BIT) | _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_START_CONDITION_BIT);
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  if (true == twi_inRepStart) {
    ...
    // send start condition
    TWI_CONTROL_REG = _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_ENABLE_ACK_BIT) | _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_START_CONDITION_BIT); // enable INTs
...
void twi_reply(uint8_t ack)
{
  ...
  if(ack){
    TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_ENABLE_ACK_BIT);
  }else{
    TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_INTERRUPT_FLAG);
  }
}
...
{
  // send stop condition
  TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_ENABLE_ACK_BIT) | _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_STOP_CONDITION_BIT);

  // wait for stop condition to be executed on bus
  // TWI_INTERRUPT_FLAG is not set after a stop condition!
  #if defined(WIRE_TIMEOUT)
    ...
    while(TWI_CONTROL_REG & _BV(TWI_STOP_CONDITION_BIT)){
...
{
  // release bus
  TWI_CONTROL_REG = _BV(TWI_ENABLE_BIT) | _BV(TWI_INTERRUPT_ENABLE) | _BV(TWI_ENABLE_ACK_BIT) | _BV(TWI_INTERRUPT_FLAG);
...
void twi_handleTimeout(bool reset){
  ...
  if (reset) {
    ...
    uint8_t previous_TWBR = TWI_BIT_RATE_REG;
    uint8_t previous_TWAR = TWI_SLAVE_ADDR_REG;
    ...
    TWI_SLAVE_ADDR_REG = previous_TWAR;
    TWI_BIT_RATE_REG = previous_TWBR;
  }
}
  ...
  switch(TW_STATUS){
    ...
    case TW_MT_DATA_ACK: // slave receiver acked data
      ...
      if(twi_masterBufferIndex < twi_masterBufferLength){
        ...
        TWI_DATA_REG = twi_masterBuffer[twi_masterBufferIndex++];
        twi_reply(1);
      }else{
  if (twi_sendStop)
          twi_stop();
    ...
    TWI_CONTROL_REG = _BV(TWI_INTERRUPT_FLAG) | _BV(TWI_START_CONDITION_BIT)| _BV(TWI_ENABLE_BIT) ;
    twi_state = TWI_READY;
  }
      }
      break;
    ...
    case TW_MR_DATA_ACK: // data received, ack sent
      ...
      twi_masterBuffer[twi_masterBufferIndex++] = TWI_DATA_REG;
      /* fall through */
  ...
  break;
    ...
    case TW_SR_GCALL_DATA_ACK: // data received generally, returned ack
      ...
      if(twi_rxBufferIndex < TWI_BUFFER_SIZE){
        ...
        twi_rxBuffer[twi_rxBufferIndex++] = TWI_DATA_REG;
        twi_reply(1);
      }else{
        // otherwise nack
        twi_reply(0);
    ...
    case TW_ST_DATA_ACK: // byte sent, ack returned
      ...
      TWI_DATA_REG = twi_txBuffer[twi_txBufferIndex++];
